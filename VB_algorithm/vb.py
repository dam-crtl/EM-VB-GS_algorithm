...
class VB_algorithm_GMM:
    ...
    def _expectation(self, X):
        ...
        for k in range(self.n_clusters):
            ...
            Lambda_tilde[k] = np.array([sum_digamma])[0][0] + self.n_feature * np.log(2) + np.log(np.linalg.det(self.W[k]))
    ...
    def _maximization(self, X):
        sum_gamma = np.sum(self.gamma, axis=0, keepdims=True).T
        sum_gamma_x = (self.gamma.T @ X) / sum_gamma
        sum_gamma_xx = np.zeros((self.n_sample, self.n_clusters, self.n_feature, self.n_feature))
        for n in range(self.n_sample):
            for k in range(self.n_clusters):
                residual = (X[n] - sum_gamma_x[k]).reshape(-1, 1)
                sum_gamma_xx[n][k] = self.gamma[n][k] * residual @ residual.T
        S = np.sum(sum_gamma_xx, axis=0) / sum_gamma.reshape((-1, 1, 1))
        self.alpha = self.alpha0 + sum_gamma
        self.beta = self.beta0 + sum_gamma
        for k in range(self.n_clusters):
            self.m[k] = (self.beta0 * self.m0 + sum_gamma[k] * sum_gamma_x[k]) / self.beta[k]
        for k in range(self.n_clusters):
            residual = (sum_gamma_x[k] - self.m0).reshape(-1, 1)
            self.W[k] = np.linalg.inv(np.linalg.inv(self.W0) + sum_gamma[k] * S[k] 
                                      + ((self.beta0 * sum_gamma[k]) / (self.beta0 + sum_gamma[k])) * residual @ residual.T)
    ...
    def _mixed_gaussian(self, X, mu, Sigma, pi):
        ...
        for i in range(self.n_clusters):
            for j in range(self.n_sample):
                output[j] += pi[i][0] * self._multivariate_gaussian(X[j], mu[i], Sigma[i])
